...
export const adminService = {
  ...
  mfaVerify: (code) => request('POST', '/auth/mfa/verify', { code }),
  ...
  fulfillmentShipments: (statusFilter = null) => request('GET', `/shipping/provider/shipments?${qs({ status_filter: statusFilter })}`),
  createProviderShipment: (orderId, data) => request('POST', `/shipping/provider/orders/${encodeURIComponent(orderId)}?${qs(data)}`),
  assignAwb: (shipmentId, courierId = null) => request('POST', `/shipping/provider/shipments/${encodeURIComponent(shipmentId)}/awb?${qs({ courier_id: courierId })}`),
  schedulePickup: (shipmentId) => request('POST', `/shipping/provider/shipments/${encodeURIComponent(shipmentId)}/pickup`),

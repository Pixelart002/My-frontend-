/**
 * Shared runtime data-type guards/coercers for the
 * frontend/backend boundary.
 *
 * Rules:
 * - IDs, slugs, codes, postal codes and phone numbers stay strings.
 * - Quantities/pages/limits are finite integers.
 * - Money/rates are finite numbers; callers decide whether null is valid.
 * - Booleans accept only real booleans or explicit true/false strings.
 * - Never use Number(value) || fallback for identifiers
 *   or optional numeric fields.
 * - Invalid values are never silently converted into valid values.
 */
...
export function asNonNegativeNumber(
  value,
  fallback = 0,
) {
  const number = asFiniteNumber(

...
import { RiArchive2Line, RiCloseLine, RiLogoutBoxRLine, RiMapPin2Line, RiMenuLine, RiSearchLine, RiSettings3Line, RiShieldStarLine, RiShoppingBagLine, RiUser3Line, RiUserLine } from '@remixicon/react';
...
export default function Header() {
  ...
  const menuLink = (to, label, className = '', Icon = null) => <Link to={to} onClick={closeAll} className={className}>{Icon && <Icon size={17} aria-hidden="true"/>}<span>{label}</span></Link>;

  const renderMobileMenu = () => {
    if (isAdminPage && isAdmin) {
      const activePanel = new URLSearchParams(location.search).get('panel') || 'dashboard';
      return <><div className="mobile-nav-section"><span className="nav-section-label">Console</span>{ADMIN_NAV.map(([key,label]) => menuLink(`/admin?panel=${key}`,label,activePanel === key ? 'is-active' : ''))}</div><div className="mobile-nav-section"><span className="nav-section-label">Storefront</span>{menuLink('/','View storefront')}</div></>;
    }
    ...
    if (isAccount || isOrders) return <><div className="mobile-nav-section"><span className="nav-section-label">Account</span>{menuLink('/account','Profile',path === '/account' ? 'is-active' : '',RiUser3Line)}{menuLink('/orders','Order history',isOrders ? 'is-active' : '',RiArchive2Line)}{menuLink('/account/addresses','Addresses',path === '/account/addresses' ? 'is-active' : '',RiMapPin2Line)}{menuLink('/account/settings','Settings',path === '/account/settings' ? 'is-active' : '',RiSettings3Line)}<button type="button" onClick={onLogout}><RiLogoutBoxRLine size={17} aria-hidden="true"/><span>Sign out</span></button></div><div className="mobile-nav-section"><span className="nav-section-label">Shop</span>{menuLink('/shop','Shop')}{menuLink('/cart',`Shopping bag${itemCount > 0 ? ` (${itemCount})` : ''}`, '', RiShoppingBagLine)}</div></>;
    if (isProduct) return <div className="mobile-nav-section"><span className="nav-section-label">Explore</span>{menuLink('/shop','Back to shop')}{menuLink('/shop?new=1','New arrivals')}{menuLink('/cart',`Shopping bag${itemCount > 0 ? ` (${itemCount})` : ''}`, '', RiShoppingBagLine)}</div>;
    if (isShop) return <><form className="mobile-search" onSubmit={onSearch} role="search"><RiSearchLine size={18}/><input name="query" placeholder="Search products" aria-label="Search products" autoComplete="off"/></form><div className="mobile-nav-section"><span className="nav-section-label">Collection</span>{menuLink('/shop','Shop all','is-active')}{menuLink('/shop?new=1','New arrivals')}</div><div className="mobile-nav-section"><span className="nav-section-label">Bag</span>{menuLink('/cart',`Shopping bag${itemCount > 0 ? ` (${itemCount})` : ''}`, '', RiShoppingBagLine)}</div><div className="mobile-nav-section"><span className="nav-section-label">Account</span>{isAuthenticated ? <>{menuLink('/account','My account', '', RiUser3Line)}{menuLink('/account/settings','Settings', '', RiSettings3Line)}</> : <>{menuLink('/login','Login', '', RiUserLine)}{menuLink('/register','Create account')}</>}</div></>;
    if (isPolicy) return <div className="mobile-nav-section"><span className="nav-section-label">Shop</span>{menuLink('/shop','Shop')}{menuLink('/cart',`Shopping bag${itemCount > 0 ? ` (${itemCount})` : ''}`, '', RiShoppingBagLine)}</div>;
    if (isAuthPage) return <div className="mobile-nav-section"><span className="nav-section-label">Luviio</span>{menuLink('/shop','Shop')}{menuLink('/cart',`Shopping bag${itemCount > 0 ? ` (${itemCount})` : ''}`, '', RiShoppingBagLine)}</div>;
    return <><div className="mobile-nav-section"><span className="nav-section-label">Discover</span>{menuLink('/shop','Shop all')}{menuLink('/shop?new=1','New arrivals')}{menuLink('/cart',`Shopping bag${itemCount > 0 ? ` (${itemCount})` : ''}`, '', RiShoppingBagLine)}</div><div className="mobile-nav-section"><span className="nav-section-label">Account</span>{isAuthenticated ? <>{menuLink('/account','My account', '', RiUser3Line)}{menuLink('/account/settings','Settings', '', RiSettings3Line)}</> : <>{menuLink('/login','Login', '', RiUserLine)}{menuLink('/register','Create account')}</>}</div></>;
  };
  ...
  return <header className="header">
    ...
    <aside id="mobile-navigation" className={`mobile-nav${mobileOpen ? ' is-open' : ''}`} aria-label="Mobile navigation" aria-hidden={!mobileOpen}><button type="button" className="mobile-nav-close" onClick={closeAll} aria-label="Close menu"><RiCloseLine size={20}/></button><div className="mobile-nav-inner">{renderMobileMenu()}</div></aside>
  </header>;
}

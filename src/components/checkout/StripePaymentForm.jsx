...
import { RiLockLine, RiCheckboxCircleLine } from '@remixicon/react';
    ...
    <div className="payment-processing-screen" role="status" aria-live="polite">
      ...
      <div className="payment-processing-steps" aria-label="Payment progress">
        <div className="payment-processing-step is-done">
          <span className="payment-processing-dot"><RiCheckboxCircleLine size={18} /></span>
  ...
  return (
    <form onSubmit={handleSubmit} className="stripe-form payment-stripe-form">
      ...
      <div className="payment-form-actions">
        <button className="btn btn-quiet payment-back-btn" type="button" onClick={onBack} disabled={processing}>Back</button>
      ...
      <p className="hint secure-hint">Payments are encrypted and processed securely by Stripe. Order {orderNumber ? `#${orderNumber}` : ''} will be confirmed after successful payment.</p>
    </form>
  );
}

...
export default function PaymentMethodModal({
  ...
  const title = hasPaymentContent ? 'Complete payment' : review ? 'Review your order' : 'Choose payment method';
  ...
  return (
    <div
      ...
      role="presentation"
      onMouseDown={(e) => {
        if (e.target === e.currentTarget && !loading && !locked) onClose?.();
      }}
    >
      <div
      ...
      >
        <header className="payment-modal-header">
          ...
          <button
            ...
            aria-label={locked ? 'Cancel order' : 'Close payment dialog'}
            onClick={handleClose}
          ...
          {!review ? (
            ...
            <div className="payment-review">
              <div className="payment-review-card">
                ...
                {address && <div className="payment-review-address"><strong>{address.full_name || 'Delivery address'}</strong><p>{address.line1}{address.line2 ? `, ${address.line2}` : ''}</p><p>{address.city}{address.state ? `, ${address.state}` : ''} — {address.postal_code}, {address.country}</p>{address.email && <p>{address.email}</p>}{address.phone && <p>{address.phone}</p>}</div>}
              </div>
        ...
        {!review && !locked && (
          <footer className="payment-modal-actions">
            ...
            <button type="button" className="btn payment-modal-primary" onClick={onContinue} disabled={!value || loading}>Continue <RiArrowRightLine size={17} /></button>
          </footer>
        )}
        ...
        {locked && !hasPaymentContent && (
          <footer className="payment-modal-actions">
            <button type="button" className="btn btn-quiet" onClick={handleClose} disabled={loading || cancellingOrder}>Cancel order</button>
          </footer>
        )}
      </div>
    </div>

      ...
      <div className="payment-modal-body">
        {isCodSuccess ? <div className="payment-success-state"><RiCheckboxCircleFill size={54} className="order-result-icon payment-success-icon" /><strong>Your COD order is confirmed.</strong><p>Order <b>#{activeOrder.orderNumber}</b> has been placed successfully. You’ll pay when the order arrives.</p></div> : !review ? <div className="payment-modal-options" role="radiogroup" aria-label="Payment method">
          <label className={`payment-option ${value === 'stripe' ? 'is-selected' : ''}`}>
            ...
            <span className="payment-option-icon"><RiBankCardLine aria-hidden="true" className="payment-icon" /></span>
          ...
          <label className={`payment-option ${value === 'cod' ? 'is-selected' : ''}`}>
            ...
            <span className="payment-option-icon"><RiCashLine aria-hidden="true" className="payment-icon" /></span>
            <span className="payment-option-copy">
          ...
          <div className="payment-review-secure"><RiShieldCheckLine aria-hidden="true" className="payment-secure-icon" /> Your address and order total are reviewed before payment.</div>
        </div>}
      </div>
      {!isCodSuccess && !review && !locked && <footer className="payment-modal-actions"><button type="button" className="btn btn-quiet" onClick={onClose} disabled={loading}>Cancel</button><button type="button" className="btn payment-modal-primary" onClick={onContinue} disabled={!value || loading}>Continue <RiArrowRightLine aria-hidden="true" className="payment-action-icon" /></button></footer>}
      {!isCodSuccess && review && !hasPaymentContent && !activeOrder && <footer className="payment-modal-actions"><button type="button" className="btn btn-quiet" onClick={onBack} disabled={loading}>Back</button><button type="button" className="btn payment-modal-primary" onClick={onContinue} disabled={loading}>{loading ? 'Preparing…' : value === 'cod' ? 'Place COD Order' : 'Continue to Payment'} <RiArrowRightLine aria-hidden="true" className="payment-action-icon" /></button></footer>}
      {isCodSuccess && <footer className="payment-modal-actions"><button type="button" className="btn payment-modal-primary" onClick={goToOrderSuccess} disabled={loading}><span>View order confirmation</span><RiArrowRightLine aria-hidden="true" className="payment-action-icon" /></button></footer>}
    </div>
  </div>;
}

...
export default function ProductCard({ product }) {
  ...
  const slug=product.slug||product.id; const price=Number(product.price)||0; const compare=Number(product.compare_price)||0; const discount=compare>price&&compare>0?Math.round(((compare-price)/compare)*100):0; const stock=Number(product.stock); const outOfStock=product.is_active===false||(Number.isFinite(stock)&&stock<=0); const name=product.name||'Product'; const category=product.categories?.name||product.category_name||'Luviio'; const images=(Array.isArray(product.images)?product.images:[]).filter(Boolean); const gallery=images.length?images:(product.image_url?[product.image_url]:[]); const safeIndex=Math.min(activeImage,Math.max(0,gallery.length-1));

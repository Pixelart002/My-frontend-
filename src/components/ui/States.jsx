...
export function EmptyState({ title = 'Nothing here yet', message, action }) {
  return (
    <div className="state" role="status">
      <div className="state-icon" aria-hidden="true"><RiStore2Line size={34} /></div>
      <div className="state-title">{title}</div>
      {message && <p className="state-message">{message}</p>}
      {action && <div className="state-action-row">{action}</div>}
    </div>
  );
}
...
export function ErrorState({ message = 'Something went wrong.', onRetry }) {
  return (
    <div className="state" role="alert">
      <div className="state-icon state-icon-alert" aria-hidden="true"><RiErrorWarningLine size={30} /></div>

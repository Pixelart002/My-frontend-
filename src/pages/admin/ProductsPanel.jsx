...
const blank = {
  ...
  low_stock_threshold: '10', weight_grams: '', hsn_code: '9988', gst_percentage: '18',
...
const toForm = (p) => {
  ...
  return {
    ...
    weight_grams: p.weight_grams != null ? String(p.weight_grams) : '', hsn_code: p.hsn_code || '9988',
...
export default function ProductsPanel({ capabilities = {} }) {
  ...
  const load = useCallback(async () => {
    setError('');
    try {
      const [p, c] = await Promise.all([adminService.listProducts({ page: 1, page_size: PAGE_SIZE }), adminService.categories()]);
  ...
  const openCreate = () => { if (!canCreate) return; setEditingId(null); setForm({ ...blank, images: [] }); setSelectedFiles([]); setEditing(true); };
  const openEdit = (p) => { if (!canUpdate) return; setEditingId(p.id); setForm(toForm(p)); setSelectedFiles([]); setEditing(true); };
  const closeEditor = () => { if (!saving) { setEditing(false); setSelectedFiles([]); } };
  ...
  const selectFiles = (e) => {
    ...
    if (incoming.some((f) => !f.type.startsWith('image/'))) return toast.error('Please choose image files only.');
    if (incoming.some((f) => f.size > 2 * 1024 * 1024)) return toast.error('Each image must be 2 MB or smaller.');
    if (form.images.length + selectedFiles.length + incoming.length > MAX_IMAGES) return toast.error(`Maximum ${MAX_IMAGES} images per product.`);
    setSelectedFiles((v) => [...v, ...incoming]);
    e.target.value = '';
  };

  const removeImage = async (index) => {
    if (!editingId || !canManageImages) return;
    ...
    } catch (e) { toast.error(e.message || 'Unable to remove image.'); }
    finally { setDeletingImage(null); }
  };

  const setPrimary = async (index) => {
    if (!editingId || index === 0 || !canManageImages) return;
    ...
    } catch (e) { toast.error(e.message || 'Unable to set primary image.'); }
  ...
  const validate = () => {
    ...
    if (form.compare_price !== '' && Number(form.compare_price) <= Number(form.price)) return 'Compare-at price must be greater than the price.';
    if (Number(form.stock) < 0) return 'Stock cannot be negative.';
    if (!GST_SLABS.includes(Number(form.gst_percentage))) return 'Select a valid GST slab.';
    if (Number(form.weight_grams) < 0) return 'Weight cannot be negative.';
  ...
  const save = async (e) => {
    ...
    let attributes;
    try { attributes = parseAttributes(form.attributes); }
    ...
    try {
      ...
      setEditing(false); setSelectedFiles([]); await load();
    } catch (e) { toast.error(e.message || 'Unable to save product.'); }
    finally { setSaving(false); }
  };

  const remove = async (p) => {
    if (!canDelete || !window.confirm(`Delete "${p.name}"? This cannot be undone.`)) return;
    setBusyId(p.id);
    try { await adminService.deleteProduct(p.id); toast.success('Product deleted.'); await load(); }
    catch (e) { toast.error(e.message || 'Unable to delete product.'); }
  ...
  return <div className="products-admin">
    <div className="admin-head">
      ...
      {canCreate && <button className="btn btn-sm" onClick={openCreate}><RiAddLine size={16} /> Add product</button>}
    </div>
    ...
    <div className="admin-table-wrap">
      ...
      {filtered.length === 0 ? <div className="admin-empty">No products match.</div> : <table className="admin-table"><thead><tr><th>Product</th><th>SKU</th><th>Price</th><th>Stock</th><th>GST</th><th>Category</th><th>Status</th>{(canUpdate || canDelete) && <th>Actions</th>}</tr></thead><tbody>
        {filtered.map((p) => { const cat = categories.find((c) => c.id === p.category_id); return <tr key={p.id}><td><div className="product-cell">{p.image_url ? <img src={p.image_url} alt={p.name || ''} loading="lazy" /> : <div className="product-thumb"><RiImageAddLine size={17} /></div>}<div><div className="td-strong">{p.name}</div><div className="td-dim">{p.slug}</div></div></div></td><td className="td-dim">{p.sku || '—'}</td><td className="td-gold">{formatMoney(Number(p.price) || 0)}</td><td>{p.stock}</td><td>{p.gst_percentage ?? '—'}%</td><td className="td-dim">{cat?.name || '—'}</td><td>{p.is_active === false ? <span className="admin-pill pill-danger">Inactive</span> : <span className="admin-pill pill-success">Active</span>}</td>{(canUpdate || canDelete) && <td><div className="btn-row">{canUpdate && <button className="icon-btn" onClick={() => openEdit(p)} title="Edit product" aria-label={`Edit ${p.name}`}><RiEditLine size={16} /></button>}{canDelete && <button className="icon-btn danger-action" onClick={() => remove(p)} disabled={busyId === p.id} title="Delete product" aria-label={`Delete ${p.name}`}><RiDeleteBinLine size={16} /></button>}</div></td>}</tr>; })}
      </tbody></table>}
    </div>
    ...
    {editing && <AdminModal className="product-editor-modal" title={editingId ? 'Edit product' : 'Add product'} sub={editingId ? 'Update catalogue information, pricing, inventory, media and SEO.' : 'Create a complete catalogue listing with search-ready metadata.'} onClose={closeEditor}>
      <form className="product-editor" onSubmit={save}>
        <div className="editor-section"><div className="editor-section-head"><div><h3>Basic information</h3><p>Customer-facing identity. Slug is generated automatically from the product name.</p></div></div>
          <div className="field-grid"><div className="field"><label htmlFor="product-name">Name *</label><input id="product-name" autoFocus required value={form.name} onChange={(e) => setField('name', e.target.value)} /></div><div className="field"><label htmlFor="product-sku">SKU</label><input id="product-sku" value={form.sku} onChange={(e) => setField('sku', e.target.value)} placeholder="Optional internal SKU" disabled={!!editingId} /></div></div>
        ...
        <div className="editor-section"><div className="editor-section-head"><div><h3>Product media</h3><p>First image is the primary catalogue image.</p></div><span className="image-count">{form.images.length + selectedFiles.length}/{MAX_IMAGES}</span></div>
          <label className="upload-drop"><RiImageAddLine size={20} /><span><strong>Add product images</strong><small>PNG, JPG or WebP · max 2 MB each</small></span><input type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={selectFiles} disabled={!canManageImages || form.images.length + selectedFiles.length >= MAX_IMAGES} /></label>
          ...
          {selectedFiles.length > 0 && <div className="selected-image-list">{selectedFiles.map((f, i) => <div className="selected-image-row" key={`${f.name}-${i}`}><span>{f.name}</span></div>)}</div>}
        </div>

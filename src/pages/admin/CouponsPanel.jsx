...
function toPayload(form, editing) {
  const payload = { type: form.type, value: Number(form.value), min_order_amount: Number(form.min_order_amount || 0), per_user_limit: Number(form.per_user_limit || 1), is_active: Boolean(form.is_active), description: form.description.trim() };
...
export default function CouponsPanel() {
  ...
  const openEdit = (coupon) => { setEditing(coupon); setForm({ ...emptyForm, ...coupon, value: String(coupon.value), min_order_amount: String(coupon.min_order_amount ?? 0), max_discount: coupon.max_discount == null ? '' : String(coupon.max_discount), usage_limit: coupon.usage_limit == null ? '' : String(coupon.usage_limit), per_user_limit: String(coupon.per_user_limit ?? 1), valid_from: coupon.valid_from ? String(coupon.valid_from).slice(0, 16) : '', valid_until: coupon.valid_until ? String(coupon.valid_until).slice(0, 16) : '' }); setFormOpen(true); };

  const save = async (event) => {
    event.preventDefault(); setSaving(true); setError('');
    try {
      if (!editing && !form.code.trim()) throw new Error('Coupon code is required.');
      const payload = toPayload(form, editing);
      if (editing) await adminService.updateCoupon(editing.id, payload); else await adminService.createCoupon(payload);
      setEditing(null); setForm(emptyForm); setFormOpen(false); await load();
  ...
  const remove = async (coupon) => {
    ...
    setError('');
    try { await adminService.deleteCoupon(coupon.id); await load(); } catch (err) { setError(err?.message || 'Unable to delete coupon.'); }
  };
  ...
  return (
    <div className="admin-panel">
      ...
      {formOpen && <form className="admin-card" onSubmit={save}>

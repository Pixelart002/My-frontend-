...
export default function ProductDetailPage() {
  ...
  return (
    <div className="page container">
      <nav className="breadcrumbs"><Link to="/">Home</Link><span>/</span><Link to="/shop">Shop</Link><span>/</span><span>{product.name}</span></nav>
      <div className="product-detail">
        <div className="gallery">
          <div className="gallery-main">{images[activeImg] ? <img src={images[activeImg]} alt={product.name || 'Product'} /> : <span className="placeholder">{(product.name || 'L').slice(0, 1)}</span>}</div>
          {images.length > 1 && <div className="gallery-thumbs">{images.map((src, i) => <button key={i} className={`thumb ${i === activeImg ? 'is-active' : ''}`} onClick={() => setActiveImg(i)} aria-label={`View image ${i + 1}`}><img src={src} alt="" /></button>)}</div>}
        </div>
        <div className="product-info">
          ...
          <div className="qty-row"><span>Qty</span><div className="qty-stepper"><button onClick={() => setQty(clampQty(qty - 1))} aria-label="Decrease quantity"><RiSubtractLine size={16} /></button><input type="number" min="1" max={Math.max(1, stock || 999)} value={qty} onChange={(e) => setQty(clampQty(e.target.value))} aria-label="Quantity" /><button onClick={() => setQty(clampQty(qty + 1))} aria-label="Increase quantity"><RiAddLine size={16} /></button></div></div>
          <div className="product-actions"><button className="btn" onClick={() => handleAdd(false)} disabled={outOfStock || adding}><RiShoppingBagLine size={16} /> {adding ? 'Adding…' : 'Add to bag'}</button><button className="btn btn-quiet" onClick={() => handleAdd(true)} disabled={outOfStock || adding}><RiShuffleLine size={16} /> Buy now</button></div>

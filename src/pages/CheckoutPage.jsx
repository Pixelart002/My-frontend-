...
export default function CheckoutPage() {
  ...
  const items = cart?.items || []; const canProceed = items.length > 0 && !cart?.has_unavailable_items;
  const selectedAddress = useMemo(() => addresses?.find((addr) => addr.id === selected) || null, [addresses, selected]);
  const couponDiscount = Number(coupon?.discount) || 0; const finalTotal = Math.max((Number(cart?.total_amount) || 0) - couponDiscount, 0);

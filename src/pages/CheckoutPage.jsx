...
export default function CheckoutPage() {
  ...
  return <div className="page container checkout">
    ...
    <div className="checkout-layout checkout-layout-refined"><div className="checkout-main">
      ...
      <section className="checkout-section"><h2>2 · Coupon</h2>{coupon ? <div className="payment-selector checkout-payment-selector"><div className="payment-selector-copy"><span className="payment-selector-label">Payment &amp; review</span><strong>{paymentMethod === 'stripe' ? 'Stripe' : paymentMethod === 'cod' ? 'Cash on Delivery' : 'Choose payment method'}</strong><small>{activeOrder ? `Order ${activeOrder.orderNumber} is active. Finish payment or cancel this order.` : 'Select your payment method, review the selected address and complete payment in the payment window.'}</small></div><button className="btn payment-selector-action" type="button" onClick={openPaymentChooser} disabled={creating || !selected || !selectedPhoneValid || !selectedCourierId || !shippingQuote || Boolean(activeOrder)}>{creating ? 'Preparing…' : 'Choose Payment Method'} <RiArrowRightLine size={17} /></button></div>

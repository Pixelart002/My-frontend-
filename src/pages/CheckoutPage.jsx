...
export default function CheckoutPage() {
  ...
  return <div className="page container checkout">
    ...
    </div><aside className="summary checkout-summary"><p className="eyebrow">Order summary</p><ul className="summary-items">{items.slice(0, 6).map((item) => <li key={item.product_id}><span>{item.name} × {item.quantity}</span><strong>{formatMoney(item.line_total)}</strong></li>)}{items.length > 6 && <li><span>+ {items.length - 6} more</span></li>}</ul><dl className="summary-lines"><div><dt>Subtotal</dt><dd>{formatMoney(cart.subtotal)}</dd></div><div><dt>Shipping</dt><dd>{shippingQuoteLoading ? 'Calculating…' : shippingQuote ? formatMoney(shippingQuote.shipping_cost) : 'Calculated at checkout'}</dd></div><div><dt>Taxes</dt><dd>{formatMoney(cart.tax_amount)}{shippingQuote ? ' + shipping GST at payment' : ''}</dd></div>{couponDiscount > 0 && <div><dt>Coupon</dt><dd>−{formatMoney(couponDiscount)}</dd></div>}<div className="total"><dt>Total</dt><dd>{formatMoney(finalTotal)}</dd></div></dl>{shippingQuoteError ? <p className="free-ship-note" role="status">{shippingQuoteError}</p> : shippingQuote && <p className="free-ship-note"><RiArrowRightLine size={15} /> Live rate: {shippingQuote.courier_name || 'Shiprocket courier'}{shippingQuote.estimated_delivery_days ? ` · ${shippingQuote.estimated_delivery_days} days` : ''}.</p>}</aside></div>
    <PaymentMethodModal open={paymentModalOpen} value={paymentMethod} onChange={(method) => { if (activeOrder) return; setPaymentMethod(method); setIntent(null); setIntentError(''); }} onClose={() => { if (!creating && !activeOrder) resetPayment(); }} onContinue={handleModalContinue} loading={creating} review={paymentReview} address={selectedAddress} total={shippingQuote ? `${formatMoney(finalTotal)} + ${formatMoney(shippingQuote.shipping_cost)} shipping + GST at payment` : 'Calculated securely at payment'} onBack={handleModalBack} activeOrder={activeOrder} onCancelOrder={requestCancelOrder} cancellingOrder={cancellingOrder}>{paymentContent || (activeOrder?.paymentMethod === 'cod' ? <div className="payment-review"><div className="payment-review-card"><RiAlertLine size={20} /><strong>COD order created</strong><p>Order <b>{activeOrder.orderNumber}</b> is reserved for you.</p></div></div> : null)}</PaymentMethodModal>

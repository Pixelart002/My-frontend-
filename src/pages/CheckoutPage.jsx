...
export default function CheckoutPage() {
  ...
  return <div className="page container checkout">
    ...
    <div className="checkout-layout checkout-layout-refined"><div className="checkout-main">
      ...
      <section className="checkout-section checkout-payment-launch"><h2>3 · Payment</h2>{intentError && <div className="form-error" role="alert">{intentError}</div>}<div className="payment-selector" style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(190px,auto)',alignItems:'center',gap:16,width:'100%',minWidth:0,padding:'18px',border:'1px solid rgba(216,173,106,.22)',borderRadius:16,background:'linear-gradient(135deg,rgba(216,173,106,.075),rgba(255,255,255,.018) 58%),#0d0d0d',boxSizing:'border-box',boxShadow:'0 12px 30px rgba(0,0,0,.16)'}}><div className="payment-selector-copy" style={{minWidth:0,display:'grid',gap:6}}><span className="payment-selector-label" style={{display:'inline-flex',alignItems:'center',gap:7,fontSize:11,fontWeight:700,letterSpacing:'.1em',textTransform:'uppercase',color:'#d8ad6a'}}>Payment &amp; review</span><strong style={{display:'block',minWidth:0,color:'#f5f0e6',fontSize:16,lineHeight:1.35,overflowWrap:'anywhere'}}>{paymentMethod === 'stripe' ? 'Stripe' : paymentMethod === 'cod' ? 'Cash on Delivery' : 'Choose payment method'}</strong><small style={{display:'block',color:'rgba(245,240,230,.62)',fontSize:12,lineHeight:1.5,overflowWrap:'anywhere'}}>{activeOrder ? `Order ${activeOrder.orderNumber} is active. Finish payment or cancel this order.` : 'Select your payment method, review the selected address and complete payment in the secure popup.'}</small></div><button className="btn" style={{width:'100%',minHeight:48,justifyContent:'center',whiteSpace:'nowrap',paddingInline:16}} type="button" onClick={openPaymentChooser} disabled={creating || !selected || !selectedPhoneValid || !selectedCourierId || !shippingQuote || Boolean(activeOrder)}>{creating ? 'Preparing…' : 'Choose Payment Method'} <RiArrowRightLine size={17} /></button>

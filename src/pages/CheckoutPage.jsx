...
export default function CheckoutPage() {
  ...
  const currentFinalCost = useMemo(() => {
    if (!shippingQuote || backendCartTotal == null) return null;
    const cartShipping = Number(cart?.shipping_cost) || 0;
    const baseWithoutCartShipping = Number(backendCartTotal) - cartShipping;
    return Math.max(0, baseWithoutCartShipping + (Number(shippingQuote.shipping_cost) || 0) - couponDiscount);
  }, [backendCartTotal, cart?.shipping_cost, shippingQuote, couponDiscount]);
  ...
  return <div className="page container checkout">
    ...
    <div className="checkout-heading page-heading compact"><h1>Complete your order.</h1><p className="checkout-subtitle">Review your address, payment method and order total before placing the order.</p></div>
    ...
    <div className="checkout-layout checkout-layout-refined"><div className="checkout-main">
      ...
      <section className="checkout-section checkout-payment-launch"><h2>3 · Payment</h2>{intentError && <div className="form-error" role="alert">{intentError}</div>}<div className="payment-selector" style={{display:'flex',flexWrap:'wrap',alignItems:'center',gap:16,width:'100%',minWidth:0,padding:'18px',border:'1px solid rgba(216,173,106,.22)',borderRadius:16,background:'linear-gradient(135deg,rgba(216,173,106,.075),rgba(255,255,255,.018) 58%),#0d0d0d',boxSizing:'border-box',boxShadow:'0 12px 30px rgba(0,0,0,.16)'}}><div className="payment-selector-copy" style={{minWidth:0,flex:'1 1 240px',display:'grid',gap:6}}><span className="payment-selector-label" style={{display:'inline-flex',alignItems:'center',gap:7,fontSize:11,fontWeight:700,letterSpacing:'.1em',textTransform:'uppercase',color:'#d8ad6a'}}>Payment &amp; review</span><strong style={{display:'block',minWidth:0,color:'#f5f0e6',fontSize:16,lineHeight:1.35,overflowWrap:'anywhere'}}>{paymentMethod === 'stripe' ? 'Stripe' : paymentMethod === 'cod' ? 'Cash on Delivery' : 'Choose payment method'}</strong><small style={{display:'block',color:'rgba(245,240,230,.62)',fontSize:12,lineHeight:1.5,overflowWrap:'anywhere'}}>{activeOrder ? `Order ${activeOrder.orderNumber} is active. Finish payment or cancel this order.` : 'Select your payment method, review the selected address and complete payment in the payment window.'}</small></div><button className="btn" style={{flex:'1 1 190px',width:'100%',minWidth:0,minHeight:48,justifyContent:'center',whiteSpace:'nowrap',paddingInline:16}} type="button" onClick={openPaymentChooser} disabled={creating || !selected || !selectedPhoneValid || !selectedCourierId || !shippingQuote || Boolean(activeOrder)}>{creating ? 'Preparing…' : 'Choose Payment Method'} <RiArrowRightLine size={17} /></button>
      ...
      <div><dt>Current cart total</dt><dd>{backendCartTotal != null ? formatMoney(backendCartTotal) : "—"}</dd></div><div className="total"><dt>Final order total</dt><dd>Confirmed at payment</dd></div>
    </dl>{shippingQuoteLoading && <p className="free-ship-note shipping-loading-note" role="status">Calculating live courier rates…</p>}{shippingQuote && <p className="free-ship-note"><RiArrowRightLine size={15} /> Selected courier: {shippingQuote.courier_name || "Shiprocket courier"}. Final cost uses the selected live shipping quote and current cart/coupon values.</p>}</aside></div>

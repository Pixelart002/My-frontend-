...
export default async function handler(req, res) {
  ...
  const rawProductImage = String(product.image_url || '').trim();
  const productImage = /^https?:\\/\\//i.test(rawProductImage) ? rawProductImage : '';
  const generatedImage = `${SITE}/api/og?slug=${encodeURIComponent(slug)}`;
  const primaryImage = productImage || generatedImage;
  ...
  const schema = {
    ...
    name,
    description,
    image: [primaryImage, generatedImage].filter((value, index, values) => values.indexOf(value) === index),
    category,
    url,
    ...
    ...(Number.isFinite(price) && price > 0
      ? {
          offers: {
            ...
            price,
            availability,
            seller: { '@type': 'Organization', name: 'Luviio', url: SITE },
          },
        }
      : {}),
  };

  const imageTags = productImage
    ? `
...
`
    : `
<meta property="og:image" content="${generatedImage}">
<meta property="og:image:secure_url" content="${generatedImage}">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:image:type" content="image/png">
<meta property="og:image:alt" content="${escapeHtml(name)}">
...
<title>${escapeHtml(name)} — Luviio</title>
<meta name="description" content="${escapeHtml(description)}">
<link rel="canonical" href="${url}">
<meta property="og:type" content="product">
<meta property="og:site_name" content="Luviio">
<meta property="og:locale" content="en_IN">
<meta property="og:title" content="${escapeHtml(name)} — Luviio">
<meta property="og:description" content="${escapeHtml(description)}">
<meta property="og:url" content="${url}">
<meta property="og:logo" content="${SITE}/og-default.svg">
${imageTags}
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="${escapeHtml(name)} — Luviio">
...
<style>body{margin:0;background:#101010;color:#f3eee7;font:16px system-ui,sans-serif}main{max-width:900px;margin:0 auto;padding:48px 24px}h1{font:48px Georgia,serif;margin:12px 0}p{color:#aaa39a;line-height:1.6}a{color:#d8ad6a}</style>
</head>
<body>
<main><small>LUVIIO</small><h1>${escapeHtml(name)}</h1><p>${escapeHtml(description)}</p><a href="${url}">Open product</a></main>
<script>window.location.replace(${JSON.stringify(url)});</script>
  ...
  res
    ...
    .setHeader('Vary', 'User-Agent, Sec-Fetch-Mode, Sec-Fetch-Dest')
    .send(html);
}
